...
ゲームとは入力情報の処理方法が異なるため、ゲーム内での入力と一致しない場合があります。

一部のゲームコントローラーの方向入力値の仕様がわからないものがあるため、ゲームコントローラーによっては正常に動作しません。  
FUSION Wired Fightpad for PlayStation 4（DP入力時）や一部のNintendo Switch対応コントローラーなどが該当します。  

### Gamepad API  

入力情報の取得はGamepad API（Web API）を使用しているため、**XInput未対応のゲームコントローラーやキーボードなどには対応しておりません**。  

Gamepad APIの仕様上、常時繰り返し処理でゲームコントローラーの情報（どのボタンが押されているかなど）を取得しているため、ループ間隔の空白の時間にのみ押されたボタンの情報は取得できません。  
通常はディスプレイや配信ソフトのフレームレート（4～36ミリ秒程度）の間隔で処理を行う仕様になっています。  
