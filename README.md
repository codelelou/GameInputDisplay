...
コントローラースキンのカスタマイズを行わない場合は本ソフトウェアをダウンロードしなくても、URLに「https://codelelou.github.io/GameInputDisplay/index.html」と設定することで使用できると思います。
...
間隔が短すぎるとパソコンに過度の負荷が掛かったり、ブラウザの制約などにより指定した間隔で動作しない場合があります。  
...
入力無視したいボタンの番号を「index.html?excludes=8,9,11」のように、半角カンマ区切りで指定します。  
...
コントローラースキンが未設定状態の場合はコントローラーが表示されない場合があります。  
...
標準では格闘ゲーム「ストリートファイターV」（カプコン）での利用を想定したキーコンフィグになっています。  

コントローラースキンが「DualPad」の場合は、自動的に「DualPad」用のキーコンフィグに切り替わります。  
...
パラメーターの「styles」を指定する時は、コントローラースキンとキーログのプリセットの両方を半角カンマ（「,」）で区切って指定してください（キーログのみだとコントローラーが表示されません）。  
...
寸法が異なる画像があるとCSSなどで表示位置の調整が必要になる場合があります。
