...
ストリートファイター6の「左右同時入力はニュートラル」と「上下同時入力はニュートラル」の方向入力ルールへのプログラム的な対応は基本的に行っていません。
キーログではCSSで対応しており、方向入力は余分に表示されやすく、トレモなどゲーム内との表示と相違が発生しやすいです。特に方向入力をボタンとレバーで併用して行う場合は余分に表示されやすくなります。
...
非純正DualShock4風コントローラーなどではボタン配置（番号）が異なる可能性があるため、正常に表示されない場合があります。  
