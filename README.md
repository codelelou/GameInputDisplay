配信画面にコントローラーのどのボタンを押しているのか（コントローラースキン）と入力履歴（キーログ）を表示するために開発しました。  

初期設定はPS4純正パッド（DualShock4）向けのコントローラースキン・キーログになっています。  

元々は自分用として格闘ゲーム「ストリートファイターV」のゲーム実況用に開発しました。  
配信ソフトに「[OBS](https://obsproject.com/ja/)」を使っているので、説明もOBS向けになっていますが、他の配信ソフトでもOBSと同等の機能があると思うので使用はできると思います。  

コントローラースキンは画像の差し替えだけでも可能（ベースの画像と押したボタンの差分画像で構成）にしています。  
設定・カスタマイズは配信ソフト「OBS」のブラウザソースのプロパティでURLパラメーター設定とカスタムCSSで行います。  
...
最大4つのゲームコントローラーの認識ができ、パラメーター設定「controller」で入力を表示することローラーを指定できます。  
...
最初はコントローラースキンの各ボタンの差分画像の読み込みに時間がかかる場合があります。事前に全てのボタンやスティックを操作して差分画像をキャッシュしておくと遅延が改善されるかと思います。  
これは標準のコントローラースキンをシンプルにしてCSS初心者でもカスタマイズしやすいように、差分画像の自動キャッシュしない作りにしているためです。  
...
パラメーター設定ジェネレーターを使うと便利です（[デバッグモード](https://codelelou.github.io/GameInputDisplay/index.html?modes=Debug)から使用できます）。  
